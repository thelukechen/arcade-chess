...
public class Pawn extends Piece {
    ...
    public void possibleMoves() {
        ...
        if (lookVertical(true) > 0) { //up one
            if (getSquare().getBoard().getSquareArr()[coordinate / 10][coordinate % 10].getPiece().getColor() == 0) {
                if (isValidMove(coordinate)) {
                    list.add(coordinate);
                }
            }
    ...
    public void addEnPassant(ArrayList<Integer> list) {
        for (int i = -1; i <= 1; i++) {
            ...
            if (lookHorizontal(right) == 1) {
                Piece piece = getSquare().getBoard().getSquareArr()[getX() + i * getColor()][getY()].getPiece();
    ...
    public Image image() {
        if (getColor() == -1) {
            return App.loadImage("/pieces/white/pawnW.png", 80, 80, true, false);
        } else {
            return App.loadImage("/pieces/black/pawnB.png", 80, 80, true, false);
        }
    }
}
